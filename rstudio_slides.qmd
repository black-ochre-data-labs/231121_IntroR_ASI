...
- Keeps a record of the last ~200 lines of code
    ...
    + Best practice is to enter + execute code from the Script Windows
...
```{r}
# x is a vector. To check how many values are in the vector
length(x)
```
...
```{r}
# I'm not sure. Are any values of x greater than one?
x > 1
```
...
```{r}
# Can I use `x` for this?
letters[x]
```

. . .

- That last step may have confused a few people <br>$\implies$ Will become clearer as we progress
...
```{r}
# Coercing x to characters will show every element with quotation marks
